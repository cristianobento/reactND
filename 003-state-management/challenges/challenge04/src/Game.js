...
class Game extends Component {
  // criando construtor. É iniciado sempre quando componente é criado/recriado
  constructor(props) {
    super(props);
    const valuesArray = this.createAQuestion(); // retorna array com os valores para o jogo
    // seta state inicial
    this.state = {
  ...
  handleAnswer = event => {
    const newValuesArray = this.createAQuestion();
    ...
    return (
      <div>
        <div className="equation">
          <p className="text">{`${value1} + ${value2} + ${value3} = ${proposedAnswer} (${corrAnswer})`}</p>
        </div>
